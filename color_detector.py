...
# 方法1: HEXカラーコードで指定(推奨)
TARGET_COLOR_HEX = "#FF0044"  # 検出したい色
# その他の例:
# "#FF0000"  # 赤
# "#00FF00"  # 緑
# "#0000FF"  # 青
    ...
    while True:
        ...
        if found:
            ...
            if detection_count >= 3 and not color_detected:  # 3フレーム連続で検出
                print(f"★ 見つけた! ({TARGET_COLOR_HEX}色 - 物体数: {object_count})")
                color_detected = True
        else:
        ...
        # 検出状態を画面に日本語で表示
        if color_detected:
            frame = put_japanese_text(frame, f"色検出! ({object_count}個の物体)", (10, 10),
                                     font_size=24, color=(0, 255, 0))
            frame = put_japanese_text(frame, f"合計面積: {int(total_area)}", (10, 45),
                                     font_size=20, color=(0, 255, 0))
        else:
            frame = put_japanese_text(frame, f"{TARGET_COLOR_HEX}を探しています...", (10, 10),
                                     font_size=24, color=(0, 0, 255))
        
        # 元の映像とマスクを表示
        cv2.imshow("Tello Camera - Color Detection", frame)
...
"""
色検出の設定:
- TARGET_COLOR_HEX: 検出したい色のHEXコード
- MIN_AREA: 検出する最小面積(ピクセル数)
- HSV範囲: rgb_to_hsv_range関数で自動生成

HSV値の調整方法:
- H (色相): 0-180 (赤=0/180, 緑=60, 青=120)
- S (彩度): 0-255 (低いと白っぽい、高いと鮮やか)
- V (明度): 0-255 (低いと暗い、高いと明るい)
- hue_range: 大きくすると似た色も検出
- sat_range: 小さくすると薄い色も検出
- val_range: 小さくすると暗い色も検出
"""
